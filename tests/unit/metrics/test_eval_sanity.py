...
def test_tusrec_gt_zero_and_shift_growth():
    ...
    # LPE is local (relative-motion) error: when all frames share the same
    # constant global offset the per-pair local transforms are unchanged,
    # so LPE correctly stays 0.  GPE captures the accumulated global drift.
    assert metrics_shift["LPE_mm"] == 0.0
