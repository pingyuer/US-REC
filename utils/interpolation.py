...
def eright_points_in_1d(x_low,x_up,y_low,y_up,z_low,z_up,xl,yl,zl,initial):
    ...
    x_low = torch.clamp(x_low, 0, xl - 1)
    y_low = torch.clamp(y_low, 0, yl - 1)
    z_low = torch.clamp(z_low, 0, zl - 1)
    x_up[x_up >= xl] = xl-1
    y_up[y_up >= yl] = yl-1
    z_up[z_up >= zl] = zl-1
    x_up = torch.clamp(x_up, 0, xl - 1)
    y_up = torch.clamp(y_up, 0, yl - 1)
    z_up = torch.clamp(z_up, 0, zl - 1)
